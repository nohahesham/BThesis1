...
public class Bswabe {
	...
	public static void setup(BswabePub pub, BswabeMsk msk) {
		...
		pub.g_hat_alpha = pairing.pairing(pub.g, msk.g_alpha); //target
	}
	...
	public static BswabePrv keygen(BswabePub pub, BswabeMsk msk, String[] attrs)
		...
		Element gr, r, beta_inv;
		Pairing pairing;

		/* initialize */
		pairing = pub.p;
		prv.d = pairing.getG1().newElement();
		gr = pairing.getG1().newElement();
	...
	public static BswabeCphKey enc(BswabePub pub, String policy)
		...
		Element s, m;   //s is the random element , m is the message element of G1
		...
		m.setToRandom();
		s.setToRandom();
    	cph.cs = pub.g_hat_alpha.duplicate(); //can i comment it?.
